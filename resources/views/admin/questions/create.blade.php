...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold text-dark mb-0">Add Question</h2>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 bg-transparent p-0">
                    ...
                    <li class="breadcrumb-item active text-primary" aria-current="page">Add Question</li>
                </ol>
            </nav>
        </div>
    ...
    <div class="glass-card border-0 shadow-sm">
        <div class="card-header bg-transparent border-bottom-0 pt-4 pb-0 px-4">
            <h5 class="fw-bold text-dark mb-0"><i class="fas fa-question-circle text-primary me-2"></i>Question Details</h5>
        </div>
            ...
            <form method="POST" action="{{ route('admin.questions.store', $exam) }}">
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control bg-light border-0 @error('question_text') is-invalid @enderror" id="question_text" name="question_text"
                        rows="4" placeholder="Enter the question text here..." required>{{ old('question_text') }}</textarea>
                    @error('question_text')
                ...
                <div class="row g-4 mb-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" class="form-control bg-light border-0" id="marks" name="marks" value="{{ old('marks', 1) }}" min="1">
                        </div>
                    </div>
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" class="form-control bg-light border-0" id="negative_marks" name="negative_marks" value="{{ old('negative_marks', 0) }}" min="0" step="0.01">
                        </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="glass-card bg-light border-0 p-3 rounded">
                        <p class="small text-muted mb-3"><i class="fas fa-info-circle me-1"></i>Fill in the options and select the radio button next to the <strong>correct answer</strong>.</p>
                        
                        <div id="options-container">
                            @for ($i = 0; $i < 4; $i++)
                                <div class="input-group mb-3 shadow-sm rounded">
                                    <div class="input-group-text bg-white border-0 border-end">
                                        <input class="form-check-input mt-0" type="radio" name="correct_option" value="{{ $i }}" {{ old('correct_option') == $i ? 'checked' : '' }} required aria-label="Correct Option">
                                    </div>
                                    <span class="input-group-text bg-white border-0 fw-bold text-primary">{{ chr(65 + $i) }}</span>
                                    <input type="text" class="form-control border-0 py-2" name="options[]" placeholder="Enter option text..." value="{{ old('options.' . $i) }}" required>
                                </div>
                            @endfor
                        </div>
                    </div>
                ...
                <div class="mb-5">
                    ...
                    <textarea class="form-control bg-light border-0" id="explanation" name="explanation" rows="3" placeholder="Provide an explanation for the correct answer, visible after submission.">{{ old('explanation') }}</textarea>
                </div>
